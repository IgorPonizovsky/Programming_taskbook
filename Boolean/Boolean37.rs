...
fn main() { 
    ...
    if ((x1 - 1) <= x2) && ((x1 + 1) >= x2) && 
       ((y1 - 1) <= y2) && ((y1 + 1) >= y2) { 
        println!("Король за один ход может перейти с одного поля на другое"); 
    } else { 
        println!("Король за один ход не может перейти с одного поля на другое"); 
    }; 
} 
