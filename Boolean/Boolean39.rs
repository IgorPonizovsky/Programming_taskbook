...
fn main() { 
    ...
    if ((x1 - x2).abs() == (y1 - y2).abs()) | 
       ((x1 == x2) | (y1 == y2)) { 
        println!("Ферзь за один ход может перейти с одного поля на другое"); 
    } else { 
        println!("Ферзь за один ход не может перейти с одного поля на другое"); 
    }; 
} 
