...
fn main() { 
    ...
    if (x1 == x2) | 
       (y1 == y2) { 
        println!("Ладья за один ход может перейти с одного поля на другое"); 
    } else { 
        println!("Ладья за один ход не может перейти с одного поля на другое"); 
    }; 
} 
