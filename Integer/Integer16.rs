// Вс 10 дек 2023 12:49:15 MSK
// Дано трёхначное число. Вывести число, полученное при перестановке цифр десятков и единиц исходного числа 
// (например, 123 -> 132) 
...
fn main() { 
    ...
    if (x/100) != 0 { 
        println!("Новое число: {}", (x/100)*100+((x%10)*10)+((x%100)/10)); 
    } else { 
        println!("Введите целое ТРЁХЗНАЧНОЕ число"); 
        process::exit(0); 
    }; 
